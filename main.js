...
// --- เริ่มการตรวจสอบตัวแปร config.js แบบละเอียด ---
console.log("--- DEBUGGING CONFIG.JS VARIABLES ---");
console.log(`Checking 'contractAddress': ${typeof window.contractAddress} (Value: ${window.contractAddress})`);
console.log(`Checking 'stakingABI': ${typeof window.stakingABI} (Value: ${window.stakingABI ? 'Defined' : 'Undefined/Null'})`);
console.log(`Checking 'usdtAddress': ${typeof window.usdtAddress} (Value: ${window.usdtAddress})`);
console.log(`Checking 'usdtABI': ${typeof window.usdtABI} (Value: ${window.usdtABI ? 'Defined' : 'Undefined/Null'})`);
console.log(`Checking 'kjcAddress': ${typeof window.kjcAddress} (Value: ${window.kjcAddress})`);
console.log(`Checking 'routerAddress': ${typeof window.routerAddress} (Value: ${window.routerAddress})`);
console.log(`Checking 'BSC_CHAIN_ID': ${typeof window.BSC_CHAIN_ID} (Value: ${window.BSC_CHAIN_ID})`);
console.log("--- END DEBUGGING CONFIG.JS VARIABLES ---");
// --- สิ้นสุดการตรวจสอบตัวแปร config.js แบบละเอียด ---
...
async function connectWallet() {
  ...
  try {
    ...
    usdtDecimals = await getTokenDecimals(usdtContract, 18); // USDT Decimals: 18 (fallback)
    kjcDecimals = await getTokenDecimals(kjcContract, 18);  // KJC Decimals: 18 (fallback)
