...
// This is here because it's a standard ABI for the router, not specific to your contracts
...
async function connectWallet() {
  ...
  try {
    ...
    document.getElementById("walletAddress").classList.add("success"); // Add success class
    document.getElementById("walletAddress").classList.remove("error");

    // --- Initializing Contracts ---
    console.log("connectWallet: Initializing contracts...");
    // **ตรวจสอบให้แน่ใจว่าตัวแปรจาก config.js ถูกโหลดและมีอยู่**
    if (typeof contractAddress === 'undefined' || typeof stakingABI === 'undefined' ||
        ...
        console.error("connectWallet: Critical: One or more contract addresses/ABIs from config.js are undefined. Please check config.js file and its loading order.");
    ...
    // ใช้ ERC20_ABI_MINIMAL สำหรับ KJC หากคุณไม่มี kjcABI ใน config.js
    ...
    usdtDecimals = await getTokenDecimals(usdtContract, 18); // USDT Decimals: 18
    kjcDecimals = await getTokenDecimals(kjcContract, 18);  // KJC Decimals: 18
    console.log(`connectWallet: USDT Decimals: ${usdtDecimals}, KJC Decimals: ${kjcDecimals}`);


    generateReferralLink();
    loadStakingInfo();
    loadReferralInfo(); // โหลดข้อมูลค่าแนะนำด้วย
...
async function buyToken() {
  ...
  try {
    ...
    if (receipt && receipt.status) {
        ...
        document.getElementById("buyTokenStatus").classList.add("success");
        loadStakingInfo();
        loadReferralInfo(); // อัปเดตข้อมูลค่าแนะนำด้วย
    } else {
...
async function claimReferralReward() {
  ...
  try {
    ...
    if (BigInt(rawClaimable) === BigInt(0)) {
        ...
        document.getElementById("referralClaimStatus").classList.add("success"); // Considered success in this context
        return;
    }
...
// Helper function สำหรับสร้างข้อความ Error ที่เป็นมิตร
...
window.addEventListener('load', () => {
  ...
  document.getElementById("claimStakeRewardBtn")?.addEventListener('click', claimReward); // แก้ไข ID ให้ตรงกับ HTML
